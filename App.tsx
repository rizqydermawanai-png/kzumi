...
import { type AppContextType, type CartItem, type User, type SiteConfig, type ShippingPackage, Category, Order, Product, Courier, ProductDiscount, PromoCode, AppNotification, Toast, BankAccount, QrisInfo, CustomOrderRequest, WarrantyClaim, Bundle, SizeChart, SentEmail } from './types';
...
import { generateOrderConfirmationEmail, generateShippingUpdateEmail, generateMassEmail } from './components/EmailTemplates';
...
const getInitialState = <T,>(key: string, defaultValue: T): T => {
  ...
  // If nothing is found, or an error occurs, this is the first load.
  // We don't save the default here, let useEffect handle initial save.
  return defaultValue;
};

// Error Boundary Component for Production Readiness
// FIX: Reverted to using a constructor for state initialization to resolve a potential TypeScript error regarding 'props' property access in class components.
class ErrorBoundary extends React.Component<{ children: React.ReactNode }, { hasError: boolean }> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', padding: '2rem', textAlign: 'center', fontFamily: "'Inter', sans-serif" }}>
          <i className="fas fa-exclamation-triangle" style={{ fontSize: '3rem', color: '#ef4444', marginBottom: '1rem' }}></i>
          <h1 style={{ fontSize: '2rem', fontFamily: "'Playfair Display', serif", marginBottom: '0.5rem' }}>Terjadi Kesalahan</h1>
          <p style={{ color: '#666', marginBottom: '1.5rem' }}>Maaf, aplikasi mengalami masalah tak terduga. Tim kami telah diberi tahu.</p>
          <button 
            onClick={() => window.location.reload()}
            style={{
                backgroundColor: '#1a1a1a', color: 'white', padding: '0.8rem 2rem', borderRadius: '8px', 
...
// FIX: Moved AdminRoute outside of the App component to follow React best practices, prevent re-renders, and resolve type errors.
// It now accepts 'user' as an explicit prop.
// FIX: Explicitly typed AdminRoute with React.FC to aid type inference and resolve missing 'children' property error.
...
const App: React.FC = () => {
  // --- START: Persisted State using localStorage ---
  const [siteConfig, setSiteConfig] = useState<SiteConfig>(() => getInitialState('KAZUMI_SITE_CONFIG', initialSiteConfig));
  const [products, setProducts] = useState<Product[]>(() => getInitialState('KAZUMI_PRODUCTS', initialProducts));
  const [users, setUsers] = useState<User[]>(() => getInitialState('KAZUMI_USERS', initialUsers));
  const [orders, setOrders] = useState<Order[]>(() => getInitialState('KAZUMI_ORDERS', mockOrders));
  const [customOrders, setCustomOrders] = useState<CustomOrderRequest[]>(() => getInitialState('KAZUMI_CUSTOM_ORDERS', mockCustomOrderRequests));
  const [warrantyClaims, setWarrantyClaims] = useState<WarrantyClaim[]>(() => getInitialState('KAZUMI_WARRANTY_CLAIMS', initialWarrantyClaims));
  const [productDiscounts, setProductDiscounts] = useState<ProductDiscount[]>(() => getInitialState('KAZUMI_PRODUCT_DISCOUNTS', initialProductDiscounts));
  const [promoCodes, setPromoCodes] = useState<PromoCode[]>(() => getInitialState('KAZUMI_PROMO_CODES', initialPromoCodes));
  const [couriers, setCouriers] = useState<Courier[]>(() => getInitialState('KAZUMI_COURIERS', courierOptions));
  const [bankAccounts, setBankAccounts] = useState<BankAccount[]>(() => getInitialState('KAZUMI_BANK_ACCOUNTS', initialBankAccounts));
  const [qrisInfo, setQrisInfo] = useState<QrisInfo | null>(() => getInitialState('KAZUMI_QRIS_INFO', initialQrisInfo));
  const [sizeCharts, setSizeCharts] = useState<SizeChart[]>(() => getInitialState('KAZUMI_SIZE_CHARTS', initialSizeCharts));
  const [sentEmails, setSentEmails] = useState<SentEmail[]>(() => getInitialState('KAZUMI_SENT_EMAILS', []));
  
  // State per session (not persisted, except for user)
  ...
  useEffect(() => { localStorage.setItem('KAZUMI_SITE_CONFIG', JSON.stringify(siteConfig)); }, [siteConfig]);
  useEffect(() => { localStorage.setItem('KAZUMI_PRODUCTS', JSON.stringify(products)); }, [products]);
  useEffect(() => { localStorage.setItem('KAZUMI_USERS', JSON.stringify(users)); }, [users]);
  useEffect(() => { localStorage.setItem('KAZUMI_ORDERS', JSON.stringify(orders)); }, [orders]);
  useEffect(() => { localStorage.setItem('KAZUMI_CUSTOM_ORDERS', JSON.stringify(customOrders)); }, [customOrders]);
  useEffect(() => { localStorage.setItem('KAZUMI_WARRANTY_CLAIMS', JSON.stringify(warrantyClaims)); }, [warrantyClaims]);
  useEffect(() => { localStorage.setItem('KAZUMI_PRODUCT_DISCOUNTS', JSON.stringify(productDiscounts)); }, [productDiscounts]);
  useEffect(() => { localStorage.setItem('KAZUMI_PROMO_CODES', JSON.stringify(promoCodes)); }, [promoCodes]);
  useEffect(() => { localStorage.setItem('KAZUMI_COURIERS', JSON.stringify(couriers)); }, [couriers]);
  useEffect(() => { localStorage.setItem('KAZUMI_BANK_ACCOUNTS', JSON.stringify(bankAccounts)); }, [bankAccounts]);
  useEffect(() => { localStorage.setItem('KAZUMI_QRIS_INFO', JSON.stringify(qrisInfo)); }, [qrisInfo]);
  useEffect(() => { localStorage.setItem('KAZUMI_SIZE_CHARTS', JSON.stringify(sizeCharts)); }, [sizeCharts]);
  useEffect(() => { localStorage.setItem('KAZUMI_SENT_EMAILS', JSON.stringify(sentEmails)); }, [sentEmails]);
  // --- END: Persisted State ---

  // Effect to restore session on page load
  useEffect(() => {
      const loggedInUserId = localStorage.getItem('KAZUMI_LOGGED_IN_USER_ID');
      if (loggedInUserId) {
          const userToRestore = users.find(u => u.id === loggedInUserId);
          if (userToRestore) {
              setUser(userToRestore);
          } else {
              // Handle case where user ID is in storage but user doesn't exist (e.g., data cleared)
              localStorage.removeItem('KAZUMI_LOGGED_IN_USER_ID');
          }
      }
  ...
  const appStateRef = useRef({ user, products, siteConfig, cart, orders, couriers });
  useEffect(() => {
    appStateRef.current = { user, products, siteConfig, cart, orders, couriers };
  }, [user, products, siteConfig, cart, orders, couriers]);
  ...
  const sendEmail = useCallback((recipient: string, subject: string, body: string) => {
    ...
    console.log("Simulating email send:", newEmail); // For debugging
    setSentEmails(prev => [newEmail, ...prev]);
  }, []);

  const sendMassEmail = useCallback((subject: string, message: string) => {
    const customerUsers = users.filter(u => u.role === 'customer' && u.email);
    customerUsers.forEach(user => {
        const { subject: emailSubject, body: emailBody } = generateMassEmail(subject, message, user);
  ...
  const login = useCallback((email: string, role: 'customer' | 'superadmin') => {
    ...
    localStorage.setItem('KAZUMI_LOGGED_IN_USER_ID', loggedInUser.id);
  }, [users]);
  ...
  const addNotification = useCallback((userId: string | 'all', message: string, options: { subject?: string } = {}) => {
    ...
    setUsers(prevUsers =>
        prevUsers.map(u => {
            if (userId === 'all' || u.id === userId || u.email === userId) {
                // Send email if a subject is provided (for single user notifications from admin)
                if (options.subject && u.role === 'customer') {
                    const { subject: emailSubject, body: emailBody } = generateMassEmail(options.subject, message, u);
                    sendEmail(u.email, emailSubject, emailBody);
                }
  ...
  const createOrder = useCallback((cartItems: any[], total: number, shipping: ShippingPackage, user: User): Order => {
    ...
    const { subject, body } = generateOrderConfirmationEmail(newOrder, user);
      ...
      setOrders(prevOrders =>
          prevOrders.map(order => {
              if (order.id === orderId) {
                  ...
                  const userToNotify = users.find(u => u.email === updatedOrder.customerEmail);
                  if (userToNotify) {
                      let message = '';
                      ...
                      if (message) addNotification(userToNotify.id, message);

                      // Send status update email
                      const { subject, body } = generateShippingUpdateEmail(updatedOrder, userToNotify);
                      if (subject && body) {
                          sendEmail(userToNotify.email, subject, body);
                      }
                  }
  ...
  }, [users, addNotification, sendEmail]);
  ...
  const fetchShippingCosts = useCallback(async (courierId: string) => {
    const { user: currentUser, products: currentProducts, siteConfig: currentSiteConfig, cart: currentCart, couriers: currentCouriers } = appStateRef.current;
    
    if (!currentUser || !currentUser.address || currentCart.length === 0) {
        return;
    }

    const courierToFetch = currentCouriers.find(c => c.id === courierId);
    if (!courierToFetch) {
...
Sediakan opsi HANYA untuk kurir berikut: ${courierToFetch.name}. Gunakan ID kurir "${courierToFetch.id}" dan URL logo "${courierToFetch.logo}". Untuk kurir ini, berikan minimal dua layanan (misal: Reguler, Express).
...
Tolong kembalikan HANYA JSON dalam format yang telah ditentukan di dalam schema. Jangan tambahkan teks atau markdown lain di luar JSON. Pastikan nama layanan dan estimasi pengiriman dalam Bahasa Indonesia.`;
    ...
    try {
        ...
        try {
            ...
            showToast(`Gagal memproses respons dari server pengiriman.`, "error");
        ...
        const shippingData: Courier[] = parsedData;
    ...
    if (orderType === 'bulk-custom') {
        setOrders(prev => prev.map(o => {
            if (o.id === orderId) {
                ...
                const userToNotify = users.find(u => u.email === updatedOrder.customerEmail);
                if (userToNotify) {
                    addNotification(userToNotify.id, `Penawaran untuk pesanan #${orderId} telah dibuat. Silakan lakukan pembayaran DP sebesar ${new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(updatedOrder.paymentTerms.downPayment)}.`);
                }
                return updatedOrder;
            }
            return o;
        ...
        setCustomOrders(prev => prev.map(o => {
            if (o.id === orderId) {
                const updatedOrder = {
                 ...
                 const userToNotify = users.find(u => u.email === updatedOrder.email);
                if (userToNotify) {
                    addNotification(userToNotify.id, `Pesanan custom Anda #${orderId} telah difinalisasi. Silakan lakukan pembayaran penuh sebesar ${new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(updatedOrder.totalPrice!)}.`);
                }
                return updatedOrder;
            }
            return o;
 ...
 const submitPaymentProof = useCallback((orderId: string, orderType: 'bulk-custom' | 'custom', paymentProof: string, paymentStage: 'down-payment' | 'final-payment' | 'full-payment') => {
  ...
  const updateWarrantyClaim = useCallback((claimId: string, status: WarrantyClaim['status'], adminNotes: string) => {
    ...
    const generateProductInfo = useCallback(async (
        ...
        try {
            ...
            console.error("Gemini API call for product info failed:", error);
    ...
    const generateColorHex = useCallback(async (colorName: string): Promise<string> => {
        ...
        try {
            ...
            console.error("Gemini API call for color hex failed:", error);
    ...
    const getColorNameFromHex = useCallback(async (hexCode: string): Promise<string> => {
        const prompt = `Apa nama umum untuk kode warna heksadesimal berikut: "${hexCode}"? Berikan hanya nama warnanya dalam Bahasa Indonesia (misal: "Biru Laut" atau "Merah Marun").`;
         try {
            ...
            console.error("Gemini API call for color name failed:", error);
  ...
  const generateRejectionReason = useCallback(async (claimDescription: string): Promise<string> => {
    ...
    try {
        ...
        console.error("Gemini API call for rejection reason failed:", error);
