                        ...
                        <div className="p-4 bg-gray-50 space-y-4">
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="form-group md:col-span-2">
                                    <label htmlFor="sizes" className="form-label">Ukuran Tersedia (pisahkan dengan koma)</label>
                                    <input 
                                        type="text" 
                                        id="sizes" 
                                        name="sizes" 
...
const SendMessageModal: React.FC<{ target: 'all' | User; onSend: (subject: string, message: string, targetId: string | 'all') => void; onClose: () => void; }> = ({ target, onSend, onClose }) => {
    const context = useContext(AppContext);
    ...
    const handleSendClick = () => {
        if (!message || !subject) {
            context?.showToast('Judul dan isi pesan tidak boleh kosong.', 'error');
            return;
        }
        onSend(subject, message, targetId);
    };
    ...
    return (
        <div className="modal open">
            <div className="modal-content modal-form-content">
                <button onClick={onClose} className="modal-close-btn">&times;</button>
                <h3 className="text-xl font-bold mb-4">Kirim Notifikasi & Email ke {targetName}</h3>
                <div className="form-group">
                    <label>Judul Email/Notifikasi</label>
                    <input value={subject} onChange={e => setSubject(e.target.value)} className="form-input" required placeholder="Contoh: Promo Spesial Akhir Pekan!"/>
                </div>
                <div className="form-group">
                    <label>Isi Pesan</label>
                    <textarea value={message} onChange={e => setMessage(e.target.value)} rows={5} className="form-textarea" required />
                </div>
                <div className="flex justify-end mt-6">
                    <button className="form-button" onClick={handleSendClick}>Kirim Pesan</button>
                </div>
            </div>
        </div>
    );
...
const RewardCustomerModal: React.FC<{ user: User; onSend: (subject: string, message: string, userId: string) => void; onClose: () => void }> = ({ user, onSend, onClose }) => {
    const { promoCodes } = useContext(AppContext)!;
    const [subject, setSubject] = useState('Hadiah Spesial Untuk Anda!');
    ...
    return (
        <div className="modal open">
            <div className="modal-content modal-form-content">
                ...
                <div className="form-group">
                    <label className="form-label">Subjek Email</label>
                ...
                <div className="flex justify-end mt-6 pt-4 border-t">
                    <button className="form-button" onClick={() => onSend(subject, message, user.id)}>
...
export const DashboardModals: React.FC<DashboardModalsProps> = ({ modalState, onClose, showNotification }) => {
    ...
    const handleSendMessage = (subject: string, message: string, targetId: string | 'all') => {
        if (targetId === 'all') {
            sendMassEmail(subject, message);
            addNotification('all', message); // Still send in-app notification
        } else {
            // addNotification handles both in-app and email for a single user
            addNotification(targetId, message, { subject });
        }
